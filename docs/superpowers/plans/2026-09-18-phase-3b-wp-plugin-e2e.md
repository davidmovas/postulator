...
A term has no modification time in WordPress, so the plugin maintains `_postulator_modified` term meta (RFC3339 UTC) from the `created_term` and `edited_term` actions. **Superseded during implementation:** the planned `COALESCE` against an activation timestamp was dropped in favour of write-on-read — `term_modified()` returns the stored meta, and when it is absent writes the current time once and returns that. It needs no activation hook (which could race WooCommerce's taxonomy registration) and no option, so `INSTALLED_OPTION` and `activate()` do not exist. `since` is applied to the resulting value in PHP after the keyset page is fetched, so a page may be short while `nextCursor` is still set.
