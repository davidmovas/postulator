...
- `Resolve(base, siteOverride, pageOverride *TemplateSpec) TemplateSpec` implements RFC 7396 merge patch.
...
- `Cannibalization(candidate Page, entity Entity, index pagemap.Index, g graph.Graph) Verdict{Allowed bool, Evidence []Evidence{PageID, Reason(same_primary_keyword|same_entity_canonical|path_conflict)}}` (as Archond `internal/modules/content/cannibalization.go`): refuses creating a page whose primary keyword or entity is already owned by another page; used by `pages.Create` and by the import preview.
