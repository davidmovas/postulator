...
describe("readQuery and writeQuery", () => {
    it("reads the default from nothing", () => {
        ...
        expect(defaultQuery).toStrictEqual({ view: "map", lens: "all", kinds: [], isolate: false });
    });

    it("round-trips a full query", () => {
        const query = { view: "outline" as const, lens: "noPage" as const, kinds: ["product", "topic"], isolate: true };
        const written = writeQuery(query).toString();
        expect(written).toBe("view=outline&lens=noPage&kinds=product%2Ctopic&isolate=1");
        expect(readQuery(new URLSearchParams(written))).toStrictEqual(query);
    });

    it("falls back on values it does not know", () => {
        const read = readQuery(new URLSearchParams("view=foo&lens=bar&kinds=product,bogus,,topic&isolate=yes"));
        expect(read).toStrictEqual({ view: "map", lens: "all", kinds: ["product", "topic"], isolate: false });
    });
