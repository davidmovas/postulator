...
interface CategoryStatsFiltersProps {
    dateRange: { from: Date; to: Date } | undefined;
    onDateRangeChange: (range: { from: Date; to: Date } | undefined) => void;
    selectedCategoryId?: number;
}

export function CategoryStatsFilters({
    dateRange,
    onDateRangeChange,
    selectedCategoryId
}: CategoryStatsFiltersProps) {
    const quickRanges = [
        { label: "7D", days: 7 },
        { label: "30D", days: 30 },
        { label: "90D", days: 90 }
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 w-full">
            <div className="flex items-center gap-4">
                <h3 className="text-lg font-semibold">
                    {selectedCategoryId ? "Category Usage" : "Categories Performance"}
                </h3>
            </div>
