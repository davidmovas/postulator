...
import { RefreshCw, ArrowLeft, Plus, Database, Trash2, Unlink2, Upload } from "lucide-react";
import { TopicRowActions } from "@/components/topics/TopicRowActions";

export interface SiteTopicsManagerProps {
  siteId: number;
  onBack: () => void;
}

export function SiteTopicsManager({ siteId, onBack }: SiteTopicsManagerProps) {
  ...
  const [strategy] = useState<string>(DEFAULT_TOPIC_STRATEGY);
  ...
  const load = async () => {
    ...
    try {
      ...
      try {
        ...
        // If backend signals an error or empty, do not surface a toast; show empty list gracefully
        setTopics([]);
      }
      try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <button className="text-muted-foreground hover:text-foreground flex items-center gap-2" onClick={onBack}>
          ...
          <p className="mt-1 text-muted-foreground text-sm">Strategy: {strategy} • Total: {topics.length} • Unused: {unusedCount}</p>
        </div>
...
export default SiteTopicsManager;
