...
func NewDeleteNodeAction(svc sitemap.Service, node *entities.SitemapNode, keywords []string) *DeleteNodeAction {
	...
	snapshot := &entities.SitemapNode{
		...
		IsModifiedLocally: node.IsModifiedLocally,
...
func (a *DeleteNodeAction) Undo(ctx context.Context) error {
	node := &entities.SitemapNode{
		...
		DesignStatus:      a.snapshot.DesignStatus,
		GenerationStatus:  a.snapshot.GenerationStatus,
		PublishStatus:     a.snapshot.PublishStatus,
		IsModifiedLocally: a.snapshot.IsModifiedLocally,
		LastError:         a.snapshot.LastError,
