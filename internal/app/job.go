...
func (a *App) CreateJob(j *dto.Job) *dto.Response[string] {
	...
	if err = a.jobSvc.CreateJob(context.Background(), entity); err != nil {
...
func (a *App) UpdateJob(j *dto.Job) *dto.Response[string] {
	...
	entity.ID = j.ID
	if err = a.jobSvc.UpdateJob(context.Background(), entity); err != nil {
		return dtoErr[string](asAppErr(err))
	}
...
func toJobEntity(d *dto.Job) (*job.Job, error) {
	...
	if d.ScheduleTime != nil && *d.ScheduleTime != "" {
		// Parse as HH:MM:SS in current location
		parsed, err := time.Parse("15:04:05", *d.ScheduleTime)
		if err != nil {
			return nil, err
		}
		// put today's date with parsed clock so repo stores a full time
		now := time.Now()
